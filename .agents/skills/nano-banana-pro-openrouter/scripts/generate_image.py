...
def resolve_output_path(filename: str, image_index: int, total_count: int, mime: str) -> Path:
    ...
    if suffix and suffix.lower() != expected_suffix.lower():
        print(
            f"Warning: filename extension '{suffix}' doesn't match returned MIME type "
            f"'{mime}'. Using '{expected_suffix}' instead."
        )
...
def load_system_prompt():
    """Load the optional image-generation system prompt."""
    skill_dir = Path(__file__).parent.parent
    template_path = skill_dir / "assets" / "SYSTEM_TEMPLATE"
