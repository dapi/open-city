...
def can_speak_ru(api_key: str, voice_id: str, model: str, voice_settings: dict, rate_limit: float):
    ...
    return (r.status_code == 200 and len(r.content) > 2048)
...
def main():
    ...
    ap.add_argument("--test-limit", type=int, default=999)
    ap.add_argument("--list-ru", dest="list_ru", action="store_true", help="Only list RU-capable voices and exit")
    ...
    for v in voices:
        ...
        if not name or not vid:
            continue
        if can_speak_ru(api_key, vid, args.model, vs, args.rate_limit):
            ru_ok.append((name, vid))
        tested += 1

    if not ru_ok:
        print("No RU-capable voices detected. Try increasing --test-limit or check your plan/keys.")
        return
    ...
    if args.list_ru and not args.write_speakers:
        return
    ...
    if args.write_speakers:
        Path(args.write_speakers).write_text(json.dumps(mapping, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"\n[OK] speakers.json written to {args.write_speakers}")
